...
interface BrandLogoProps {
  size?: number;
  /** Unique prefix for SVG gradient IDs (avoids collisions when multiple logos render) */
  idPrefix?: string;
}

/**
 * Brand Logo — shield + radar motif with animated pulse rings and sweep.
 * Ported from client/src/components/Logo.tsx for use in Remotion scenes.
 */
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  const sweep = `${idPrefix}-sweep`;
  const clip = `${idPrefix}-clip`;
  const pulseClass = `${idPrefix}-pulse`;
  const pulseDelayedClass = `${idPrefix}-pulse-delayed`;
  const sweepClass = `${idPrefix}-sweep-anim`;
    ...
    >
      <defs>
        <style>{`
          ...
          @keyframes ${sweepClass} {
            0% { opacity: 0.6; transform: scale(0.92); }
            50% { opacity: 1; transform: scale(1); }
            100% { opacity: 0.6; transform: scale(0.92); }
          }
          .${pulseClass} {
            animation: ${pulseClass} 3s ease-out infinite;
          }
          .${pulseDelayedClass} {
            animation: ${pulseClass} 3s ease-out 1.5s infinite;
          }
          .${sweepClass} {
            animation: ${sweepClass} 4s ease-in-out infinite;
            transform-origin: 90px 92px;
          }
        `}</style>
        <linearGradient id={fill} x1="90" y1="10" x2="90" y2="190" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#1A3660" />
          <stop offset="100%" stopColor="#0C1B30" />
        </linearGradient>
        <linearGradient id={stroke} x1="90" y1="10" x2="90" y2="190" gradientUnits="userSpaceOnUse">
        ...
        <radialGradient id={glow} cx="90" cy="92" r="40" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#00CFFF" stopOpacity={0.25} />
          <stop offset="100%" stopColor="#00CFFF" stopOpacity={0} />
        </radialGradient>
        <linearGradient id={sweep} x1="90" y1="52" x2="130" y2="92" gradientUnits="userSpaceOnUse">
      ...
      <g clipPath={`url(#${clip})`}>
        {/* Core glow */}
        <circle cx={90} cy={92} r={40} fill={`url(#${glow})`} />
        ...
        <g transform="rotate(-30, 90, 92)" className={sweepClass}>
          <path d="M90 92 L90 52 A40 40 0 0 1 124.6 72 Z" fill={`url(#${sweep})`} />
        </g>

        {/* Animated pulse rings */}
        <circle cx={90} cy={92} r={28} stroke="#00CFFF" strokeWidth={0.8} fill="none" className={pulseClass} />
        <circle cx={90} cy={92} r={28} stroke="#00CFFF" strokeWidth={0.6} fill="none" className={pulseDelayedClass} />
