 ...
 */
function filterAiResponse(response: string, checkpoints: { correctAnswer: any; explanation: any; options: any }[]): string | null {
  const lower = response.toLowerCase();

  // Check for phrases that indicate the AI is giving away answers
  ...
  // Check if response contains exact correctAnswer strings from checkpoints
  for (const cp of checkpoints) {
    ...
    // Check if explanation text is being leaked
  ...
  // Check for JSON or structured data leaks
