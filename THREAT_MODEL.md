...
| ~~**RR-4**~~ | T-04 | ~~AI input filter without Unicode normalization~~ | ~~Medium~~ | **Mitigated** — `normalizeForPatternMatch()` applies NFKC normalization, strips zero-width/invisible characters, and maps 50+ Cyrillic/Greek confusables to Latin equivalents before pattern matching in both `filterAiInput` and `sanitizePrompt` |
